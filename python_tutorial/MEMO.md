...
# [12.2. 仮想環境の作成](https://docs.python.org/ja/3/tutorial/venv.html#creating-virtual-environments)
仮想環境とは、特定のバージョンの Python と幾つかの追加パッケージを含んだ Python インストールを構成するディレクトリ.
## `venv`
* 仮想環境の作成と管理を行うモジュールでありpython公式が採用している.
* 仮想環境内のpythonバージョンは仮想環境作成時点で使われているバージョン.
* 環境毎にパッケージを使い分けたいときに使われることが多い.
```
# 仮想環境作成(カレントディレクトリに仮想環境ディレクトリ`.venv`が作成される)
python3 -m venv .venv
...
```
## `pyenv`
* 複数のpythonをインストールして使い分けることができる.
* pythonバージョンを切り替えたいときに使われることが多い.（仮想環境の作成もできる）
```
